...
- [Claude Code Agent](./docs-internal/claude_code_agent.md) — hub; then [Agent Architecture](./docs-internal/claude_code_agent_architecture.md), [CLI Agent Framework](./docs-internal/cli_agent_framework.md), [Interactive Mode](./docs-internal/claude_code_interactive_mode.md), [Canonical Patterns RFC](./docs-internal/cli_agent_canonical_patterns_rfc.md)
- [LiteLLM Router Migration RFC](./docs-internal/litellm_router_migration_rfc.md) — **superseded** design history; LiteLLM is not a dependency and never shipped
...
- [WhatsApp Integration](./docs-internal/whatsapp_integration.md) (personal account, unofficial bridge) · [WhatsApp Business Service](./docs-internal/whatsapp_business_service.md) (official Meta Cloud API — **a different plugin**, see below)
- [Discord Service](./docs-internal/discord_service.md) — bot integration, multi-account over the `session_id` credential column, plugin-owned REST
- [Data Node & Vision](./docs-internal/data_node.md) — the `dataSource` agent tool, the machine-wide `data_mounts` allowlist, and the `visionAnalyze` delegate
- [Android Services](./docs-internal/android_services.md) · [Config Node Architecture](./docs-internal/config_nodes.md)
...
- **Backend NodeSpec is the single source of truth.** Plugins live in [`server/nodes/<group>/<name>.py`](./server/nodes/) and auto-register via `BaseNode.__init_subclass__`. Authoritative node count comes from the registry (see "Implemented Node Types" below), not from a glob. The plugin folders are: agent / android / browser / chat / cloudflare / code / context / discord / document / email / filesystem / gcloud / github / google / location / microsoft / model / proxy / scheduler / scraper / search / skill / social / speech / stripe / telegram / text / tool / translate / trigger / twitter / utility / vercel / vision / whatsapp / whatsapp_business / workflow. (`nodes/sarvam/` was retired into `speech/` + `translate/`; only `sarvamChatModel` under `nodes/model/` remains vendor-named.)
...
- **Icon + color** resolve per-plugin folder first (`<plugin>/icon_<nodeType>.svg` → `<plugin>/icon.svg` → the plugin's own `meta.json`), with `visuals.json` as the legacy central fallback for plugins that predate co-location. Do not declare `icon` / `color` as class attributes on a node. Three traps: **a file always beats a `meta.json` ref**, so a folder shipping `icon.svg` shadows per-node refs for every node type in it; `lucide:` refs match the package's **export** identifiers (`lucide:CheckCheck` resolves, kebab-case `lucide:check-check` silently renders nothing); and a missing ref renders **nothing** rather than erroring. Both sides are locked by tests (`client/src/assets/icons/index.test.ts` walks every `meta.json`; `test_node_spec.py` asserts every `icons` key is a registered node type).
...
**Palette groups are declared in [`server/nodes/groups.py`](./server/nodes/groups.py) — that file is the SSOT, read it rather than trusting a list here.** Each `register_group(key=...)` sets the label, icon, color, and a `visibility` bucket (`normal` = simple-mode palette, `dev` = pro/dev mode only, `all`).

**A plugin's folder name is not necessarily its palette group.** Notable divergences: `stripe/` → `payments`; `github/` → `vcs`; `cloudflare/` + `gcloud/` + `vercel/` → `deployment`; `speech/` + `translate/` → `language`; `scraper/` also uses `api`. `whatsapp_business/` keeps its **own** group, registered deliberately separate from `whatsapp` — the two share no node type, credential or palette section. Groups `ai`, `service`, and `memory` have no matching folder at all.
...
**Conditional edges are evaluated on the Temporal path too, as of the `machina-conditional-edges-v1` patch.** They previously were not — `MachinaWorkflow._find_ready_nodes` scheduled purely on dependency completion, so a condition set in the editor rendered an edge label and then did nothing once execution routed through Temporal, silently running both branches. The skip mirrors `WorkflowExecutor._evaluate_incoming_conditions` exactly: **OR-any** across a node's conditional incoming edges, and **skipping is not transitive** (a skipped node counts as completed). `evaluate_condition` is imported from `services.execution.conditions` **directly, never via the package `__init__`**, which would drag the executor / cache / recovery sweeper into the sandbox; the module itself is pure, so it is safe inside a workflow. The `workflow.patched(...)` gate is mandatory — the skip changes which activities get scheduled, so an ungated change breaks replay for in-flight workflows. Locked by `tests/temporal/test_conditional_edges.py`.
...
The journal records, it never reconstructs. `agent.execute_llm_step` writes each turn from the exact message list it hands to `ChatUnifier.chat` (`_journal_llm_turn`), immediately after the call returns. `agent.prepare_context` deliberately journals nothing — it runs before the request exists, so anything it wrote would be assembled from configuration. Journal operation ids derive from the per-firing `context_execution_id`, **not** `execution_id`: the latter is generation-scoped, so every message in a generation minted identical ids and the store's idempotency guard silently discarded every turn after the first. They are **also scoped by `agent_node_id`** (`f"{journal_operation_id}:{agent_node_id}"`) — two agents wired to one Context node resolve to the same thread, so a firing-only id made them collide on `(thread, operation_id)` and one agent's turns vanished as replays.

**`continue_as_new` carries the live transcript, it does not replay the journal.** `AgentWorkflow` rolls over under history pressure only when `context_ref` is set, and a resumed run adopts `resume["transcript"]` before its first LLM step. The earlier design rebuilt the conversation from the store on resume and shipped a transcript missing every tool result, so it ended on an assistant tool-call turn with no answers — which providers reject outright (Gemini 400: "Requests ending with a model turn are not supported"). Carrying the messages makes that unrepresentable; the retired `agent.reconstruct_context_messages` step must not come back. The rollover argument shares Temporal's 2 MiB payload limit, so an oversized transcript degrades to the opening prompt with a warning rather than failing the rollover that was trying to save the run. Locked by `tests/temporal/test_agent_workflow.py`.

**Live viewing is emitted at the persistence boundary, not per caller.** `AgentContextStore` is the one place every writer passes through (in-process loop, Temporal LLM activity, CLI-agent bridge), so it announces durable commits through the fanout registry in [`services/agent_context/listeners.py`](./server/services/agent_context/listeners.py) — `register_context_commit_listener` / `notify_context_commit` — and `nodes/context/__init__.py` registers the broadcaster. Same shape and same reason as the plugin registries: the store must never import `nodes/`. Three properties are load-bearing: the notification fires **after commit and after the ref reload** (never ahead of the state it describes); the **idempotent-replay path emits nothing** (a retried activity is not a state change); and a listener **can never fail a commit** — these commits run inside the LLM activity's heartbeat-silent post-send window on a `maximum_attempts=1` policy, so a throwing or slow listener would kill a run over a UI notification. Epoch rotation deliberately stays outside the store: `start_epoch`'s callers already emit `context.epoch.started` with a `reason` the store cannot know.

The three Context events (`context.updated` / `context.compacted` / `context.epoch.started`) **broadcast directly via `get_status_broadcaster()`, not through `services.events.dispatch.emit`** — `emit` runs a Temporal Visibility query to find workflow consumers, and no node type registers a canary consumer for `com.opencompany.context.*`, so that query is guaranteed to match nothing once per journal append. Payloads are identity + revision only; the panel refetches through the authorized `get_agent_context` handler, so the broadcast grants no read access on its own. Frontend consumption follows the `skill_lifecycle` precedent — three `case` labels in `WebSocketContext.tsx` invalidate `['agentContext']`, and the panel contains no subscription code.
...
**Two unrelated plugins.** `server/nodes/whatsapp/` is a *personal* account over an unofficial Go bridge (below). `server/nodes/whatsapp_business/` is the official **Meta Cloud API** plugin — its own palette group, sharing no node type or credential with the other: `whatsappBusinessSend` (Meta models message type as a field on one `/messages` endpoint, so text / media / template / interactive are operations, not nodes), `whatsappBusinessMedia`, and the `Receive` / `Status` triggers (separate because a canary trigger registers exactly one CloudEvents type). `phone_number_id` is deliberately **absent from Params** — on a dual-purpose ActionNode `execute_as_tool` merges `{**node_params, **tool_args}`, so a declared field is model-settable and a prompt injection could pick the sending identity; it comes from the credential via `resolve_phone_number_id`. Webhook rides the shared catch-all via `register_webhook_source`, `X-Hub-Signature-256` HMAC fails closed, and status dedup ids are composite `<wamid>:<status>` so `sent → delivered → read` don't collapse into one event. Full reference: **[WhatsApp Business Service](./docs-internal/whatsapp_business_service.md)**.

The personal-account plugin: square-design nodes with embedded QR viewing; all requests proxy through the Python backend to the WhatsApp RPC service (`WHATSAPP_RPC_PORT`). Supports individual chats, groups, and newsletter channels; all 14 WhatsApp events handled. The backend helpers, frontend component, the critical bug fixes (dependency-injection wiring, unhandled JSON parse / HTTP status errors, missing `HTTPException` re-raise), the canonical error-handling pattern, and group/sender name persistence are in **[WhatsApp Integration](./docs-internal/whatsapp_integration.md)**.
