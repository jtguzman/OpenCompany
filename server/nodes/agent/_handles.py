...
STD_AGENT_HINTS = {
    **STD_SIZE,
    "hasSkills": True,
    # An execution capability, not a rendering heuristic.  Graph
    # normalization and validation use this declaration to pair the agent
    # with exactly one system-managed Context node.
    "requiresContext": True,
}


def std_agent_handles() -> tuple:
    """Skill + Tool + Context + Task + Input + Output — shared by 18
    ...
    return (
        ...
        {"name": "input-context", "kind": "input", "position": "left", "offset": "50%", "label": "Context", "role": "context"},
...
def team_lead_agent_handles() -> tuple:
    """Skill / Tool / Teammates bottom + Context/Task/Input left + Outputs.
